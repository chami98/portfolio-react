(function () {
  ...
  function doAnalytics(userId, type, extraData) {
    if (testing) {
      return true;
    }
  ...
  function handleLinkClick(userId, link) {
    if (testing) {
  ...
  function getDefaultDescForSection(type, defaultValue) {
    if (defaultValue) {
      return defaultValue;
    }
  ...
  function getProperDate(month, year) {
    var date = "";
    if (month) {
      date += month + " ";
    }
  ...
  const viewInfo = {
    ...
    doAnalytics,
    handleLinkClick,
    getDefaultDescForSection,
    getProperDate,
  };

  root.viewInfo = viewInfo;
}).call(this);
